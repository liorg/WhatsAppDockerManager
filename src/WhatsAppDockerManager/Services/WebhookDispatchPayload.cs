    ...
    public async Task DispatchAsync(Guid phoneId, Guid contactId, Guid messageId, bool direction)
    {
        try
        {
            _logger.LogInformation(
                "[DISPATCH] phone={PhoneId} contact={ContactId} message={MessageId} direction={Direction}",
            ...
            var priority = new[]
            {
                WebhookRegistrationType.Trigger,
                WebhookRegistrationType.Job,
                WebhookRegistrationType.Recording,
            };

            foreach (var eventType in priority)
            {
                var registrations = await _supabase.GetActiveWebhooksByTypeAsync(eventType);
                if (!registrations.Any())
                    continue; // ← היה return — עוצר הכל בטעות
                ...
                foreach (var reg in registrations)
                {
                    try
                    {
                        var response = await http.PostAsync(
                            reg.CallbackUrl, // ← היה reg.CallbackUrl
                            new StringContent(payload, Encoding.UTF8, "application/json"));

                        _logger.LogInformation("[DISPATCH][{Type}] → {Url} status={Status}",
                            eventType.ToUpper(), reg.CallbackUrl, (int)response.StatusCode);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogWarning(ex, "[DISPATCH][{Type}] Failed to notify {Url}",
                            eventType.ToUpper(), reg.CallbackUrl);
                    }
                }
            }
        }
