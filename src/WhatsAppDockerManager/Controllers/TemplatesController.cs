...
public class TemplatesController : ControllerBase
{
    ...
    // ── health — מאשר שה-controller נכלל ב-build הפרוס ────────────────────────
    // נתיב מוחלט בכוונה: בלי phoneId, כדי שאפשר יהיה לבדוק בלי טלפון קיים.
    //   curl -s http://127.0.0.1:5000/api/templates/health
    // 404 = ה-controller חסר ב-DLL. 200/401 = הוא שם.
    [HttpGet("/api/templates/health")]
    [Microsoft.AspNetCore.Authorization.AllowAnonymous]
    public IActionResult Health()
    {
        var asm = typeof(TemplatesController).Assembly;
        return Ok(new
        {
            ok         = true,
            controller = nameof(TemplatesController),
            version    = asm.GetName().Version?.ToString(),
            builtAt    = System.IO.File.GetLastWriteTimeUtc(asm.Location),
            routes     = new[] { "POST /api/phones/{phoneId}/templates", "GET /api/phones/{phoneId}/templates/{templateId}" },
        });
    }
